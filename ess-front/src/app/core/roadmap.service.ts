...
export class RoadmapService {
  private readonly roadmapsSubject = new BehaviorSubject<RoadmapListItemDto[]>([]);
  private readonly selectedRoadmapSubject = new BehaviorSubject<RoadmapDetailDto | null>(null);
  private readonly loadingSubject = new BehaviorSubject<boolean>(false);
  private readonly errorSubject = new BehaviorSubject<string | null>(null);

  readonly roadmaps$: Observable<RoadmapListItemDto[]> = this.roadmapsSubject.asObservable();
   ...
   * Load all roadmaps
   */
  loadRoadmaps(): Observable<RoadmapListItemDto[]> {
  ...
  updateRoadmapNode(roadmapId: number, nodeId: number, dto: UpdateRoadmapNodeDto): Observable<RoadmapNodeDto> {
    ...
    return this.apiService.updateRoadmapNode(roadmapId, nodeId, dto).pipe(
      tap(
        () => {
          this.loadingSubject.next(false);
        },
   ...
   */
  removeRoadmapNode(roadmapId: number, nodeId: number): Observable<void> {
    this.loadingSubject.next(true);
    this.errorSubject.next(null);

    return this.apiService.deleteRoadmapNode(roadmapId, nodeId).pipe(
      tap(
        () => {
          this.loadingSubject.next(false);
        },
   ...
   */
  getRoadmapsList(): RoadmapListItemDto[] {
